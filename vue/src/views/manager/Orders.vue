<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.order_no" style="width: 300px; margin-right: 10px" placeholder="请输入订单编号查询"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe>
        ...
        <el-table-column label="下单人" prop="user_name"></el-table-column>
        <el-table-column label="订单状态" prop="status">
          <template #default="scope">
            <el-tag v-if="scope.row.status === '待支付'" type="warning">待支付</el-tag>
            <el-tag v-if="scope.row.status === '待发货'" type="success">待发货</el-tag>
            <el-tag v-if="scope.row.status === '待收货'" type="info">待收货</el-tag>
            <el-tag v-if="scope.row.status === '已收货'" type="success">已收货</el-tag>
        ...
        <el-table-column label="操作" header-align="center" width="160">
          <template #default="scope">
            <el-button v-if="scope.row.status === '待支付'" type="danger" @click="changeStatus(scope.row, '已取消'), candleOrder(scope.row.goods_id, scope.row.num)">取消</el-button>
            <el-button v-if="data.user.role === 'USER' && scope.row.status === '待支付'" type="danger" @click="changeStatus(scope.row, '待发货')">支付</el-button>
            <el-button v-if="data.user.role === 'ADMIN' && scope.row.status === '待发货'" type="danger" @click="changeStatus(scope.row, '待收货')">发货</el-button>
            <el-button v-if="data.user.role === 'USER' && scope.row.status === '待收货'" type="danger" @click="changeStatus(scope.row, '已收货')">收货</el-button>
