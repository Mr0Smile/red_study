<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.goods_name" style="width: 300px; margin-right: 10px" placeholder="请输入书本名称查询"></el-input>
    ...
    <el-dialog title="进货信息" width="40%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="100px" style="padding-right: 50px">
        <el-form-item label="书籍名称" prop="name">
          <el-select v-model="data.form.goods_id" placeholder="请选择书籍" style="width: 60%">
            <el-option
        ...
        <el-form-item label="进货数量" prop="num">
          <el-input-number :min="1" v-model="data.form.num" autocomplete="off"/>
        </el-form-item>
        <el-form-item label="进货渠道" prop="channel">
          <el-input v-model="data.form.channel" autocomplete="off" placeholder="请输入进货渠道" />
        </el-form-item>
        ...
        <el-form-item label="备注" prop="comment">
          <el-input v-model="data.form.comment" autocomplete="off" type="textarea" placeholder="请输入备注" />
        </el-form-item>
      </el-form>
      <template #footer>
...
// 分页查询
const load = () => {
  request.get('/goodsIn/selectPage', {
    params: {
...
// 新增保存
const add = () => {
  request.post('/goodsIn/add', data.form).then(res => {
    if (res.code === '200') {
      load()
...
// 编辑保存
const update = () => {
  request.put('/goodsIn/update', data.form).then(res => {
    if (res.code === '200') {
      load()
      ElMessage.success('操作成功')
      data.formVisible = false
...
const handleDelete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/goodsIn/delete/' + id).then(res => {
      if (res.code === '200') {
        load()
        ElMessage.success('操作成功')
      } else {
